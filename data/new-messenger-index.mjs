...
// ---- OTel: capture prudente des POST data ----
const SENSITIVE_KEYS = ["password","token","authorization","card_number","email"];
function redact(obj = {}) {
  try {
    const out = { ...obj };
    for (const k of SENSITIVE_KEYS) if (k in out) out[k] = "[REDACTED]";
    // Évite d'exploser la taille des spans
    const s = JSON.stringify(out);
    return s.length > 512 ? s.slice(0, 512) + "…[truncated]" : s;
  } catch { return ""; }
}
app.use((req, _res, next) => {
  if (req.method === "POST") {
    ...
    if (span) {
      ...
      // Faible cardinalité : présence des champs + taille
      span.setAttributes({
        "http.request.method": "POST",
        "app.post.fields_present": fields.sort().join(","), // ex: "content,index,user_id"
        "app.post.size_bytes": Buffer.byteLength(JSON.stringify(req.body || {})),
      });
      // Échantillon redigé, limité
      const safe = redact(req.body);
      if (safe) span.addEvent("post_body_captured", { "app.post.sample": safe });
    }
  }
  next();
});
